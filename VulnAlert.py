...
class VulnAlert:
    def __init__(self):
        ...
        parser.add_argument('--service',
                            help="If the script is being used as a service use this parameter in conjunction with "
                                 "--timeleep",
                            action='store_true',
                            dest="argservice")
...
class Ingestor:
    def __init__(self, args: object):
        ...
        if self.config.general['proxy']['active']:
            self.logger.info(
                "O proxy está ativo e as informações de proxy que estão no arquivo de configurações serão utilizadas.")
